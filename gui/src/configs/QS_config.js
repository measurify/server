...
export default function QS_configuration() {
  const base_api_url = "https://qdb.hi-drive-cdb.eu/v1";
  //operation pages
  //those are the pages of operations performed on experiments,
  // which are: updatehistory, downloadexperiment, removesteps
  const operationPages = ["uploadquestionnaire", "downloadquestionnaire"];
  ...
  restrictionPages["uploadquestionnaire"] = ["admin", "provider"];
  restrictionPages["downloadquestionnaire"] = ["admin", "analyst"];
  ...
  viewFields["features"] = [
    { items: ["name", "type", "unit", "dimension", "range"] },
    "tags",
  ];
