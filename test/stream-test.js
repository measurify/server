...
describe("Thing stream", () => {
  it("it should GET measurements of a thing", async () => {
    ...
    const device = await factory.createDevice("test-device-1", provider, [
      feature,
    ]);
    ...
    const measurement_request = {
      ...
      device: device._id,
      thing: thing._id,
      samples: factory.createSamples(1),
    };
    ...
    client.on("open", async function (message) {
      await chai
        .request(server)
        .keepOpen()
        .post("/v1/measurements")
        .set("Authorization", provider_token)
        .send(measurement_request);
    });
  ...
  it("it should GET measurements of two different things on two different socket", async () => {
    ...
    client_1.on("error", function (message) {
      assert.fail(message);
    });
    ...
    client_2.on("error", function (message) {
      assert.fail(message);
    });
    client_1.on("open", async function (message) {
      await chai
        .request(server)
        .keepOpen()
        .post("/v1/measurements")
        .set("Authorization", provider_token)
        .send(measurement_request_1);
    });
    client_2.on("open", async function (message) {
      await chai
        .request(server)
        .keepOpen()
        .post("/v1/measurements")
        .set("Authorization", provider_token)
        .send(measurement_request_2);
    });
    const promise_1 = new Promise((done_1) => {
      client_1.on("close", done_1);
    });
    const promise_2 = new Promise((done_2) => {
      client_2.on("close", done_2);
    });
