...
describe('Access read a list of constraints', () => {
    it('it should get all the public/private constraints as admin or analyst', async () => {
        ...
        let res = await chai.request(server).get('/v1/constraints/').set("Authorization", await factory.getUserToken(user_admin));
        res.should.have.status(200);
        res.body.docs.should.be.a('array');
        res.body.docs.length.should.be.eql(8);
        res = await chai.request(server).get('/v1/constraints/').set("Authorization", await factory.getUserToken(user_analyst));
        res.should.have.status(200);
        res.body.docs.should.be.a('array');
        res.body.docs.length.should.be.eql(8);
    });
    ...
    it('it should get just his own or public constraints as provider', async () => {
        ...
        let res = await chai.request(server).get('/v1/constraints/').set("Authorization", await factory.getUserToken(user_provider));
        res.should.have.status(200);
        res.body.docs.should.be.a('array');
        res.body.docs.length.should.be.eql(7);
        res = await chai.request(server).get('/v1/constraints/').set("Authorization", await factory.getUserToken(user_admin));
        res.should.have.status(200);
        res.body.docs.should.be.a('array');
    ...
    it('it should get a filtered list of his own or public measurements as provider', async () => {
        ...
        let res = await chai.request(server).get('/v1/constraints?filter=' + filter).set("Authorization", await factory.getUserToken(user_provider));
        res.should.have.status(200);
        res.body.docs.should.be.a('array');
        res.body.docs.length.should.be.eql(1);
        res = await chai.request(server).get('/v1/constraints?filter=' + filter).set("Authorization", await factory.getUserToken(user_admin));
        res.should.have.status(200);
        res.body.docs.should.be.a('array');
