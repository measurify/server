...
| Feature  | Medical indicators. For instance: breath rate, heart rate, body temperature, etc.  | Measured quantity. For instance: vehicle-coil displacement, charging parameters (with dimensions: current, voltage, speed, etc.)  | Measured quantity (e.g., Trip PI, DSI Datapoint). Dimensions include: average speed, time headway at maximum speed, percentage time in each type of driving scenario, etc.|
| Device  | Different types of medical devices (e.g., heart pulse sensor, blood pressure sensor)  | Vehicle-coil alignment system, Dynamic Wireless Charger (DWC) road and vehicle side  | The L3Pilot data toolchain  |
| Thing  | Patient  | Passage of an electric vehicle in the charging road lane   | Trip (i.e., the unit of the information source processed by the toolchain)  |
